...
def visualize_spectrograms(original_specs, reconstructed_specs, audio_names, is_vae=True, save_path=None):
    ...
    fig = plt.figure(figsize=(18, 4 * n_samples))  # Increased figure size
    
    model_type = "VAE" if is_vae else "AE"
    plt.suptitle(f'Original vs {model_type} Reconstructed Spectrograms', fontsize=20)
    
    for i in range(n_samples):
        # Original spectrogram
        plt.subplot(n_samples, 2, 2*i + 1)
        img = librosa.display.specshow(original_specs[i], x_axis='time', y_axis='mel', sr=44100, 
                                hop_length=256, cmap='viridis')
        cbar = plt.colorbar(img, format='%+2.0f dB')
        cbar.ax.tick_params(labelsize=12)
        plt.title(f'Original: {audio_names[i]}', fontsize=16)
        ...
        img = librosa.display.specshow(reconstructed_specs[i], x_axis='time', y_axis='mel', sr=44100,
                                hop_length=256, cmap='viridis')
        cbar = plt.colorbar(img, format='%+2.0f dB')
        cbar.ax.tick_params(labelsize=12)
        plt.title(f'Reconstructed ({model_type})', fontsize=16)
        plt.xlabel('Time (s)', fontsize=14)
        plt.ylabel('Mel Frequency', fontsize=14)
        plt.xticks(fontsize=12)
        plt.yticks(fontsize=12)
...
def compare_artifact_assessment(artifact_scores, save_path=None):
    ...
    ax.set_ylabel('Artifact Severity Score (0-5)', fontsize=16)
    ax.set_title('Audio Artifact Assessment by Model Type', fontsize=18)
    ax.set_xticks(x)
    ...
    if save_path:
        output_path = os.path.join(OUTPUT_FOLDER, save_path)
        plt.savefig(output_path, dpi=300, bbox_inches='tight')
        print(f"Artifact assessment visualization saved to {output_path}")
...
if __name__ == "__main__":
    ...
    visualize_spectrograms(orig_specs[2:], reconstructed_specs_vae[2:], sample_names[2:],is_vae=True, save_path="vae_spectrogram_comparison.png")
